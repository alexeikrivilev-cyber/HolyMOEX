...
latest_portfolio AS (
  SELECT
    (SELECT portfolio_id FROM portfolio.portfolio_snapshot ORDER BY created_at DESC LIMIT 1) AS portfolio_id,
    (SELECT created_at FROM portfolio.portfolio_snapshot ORDER BY created_at DESC LIMIT 1) AS created_at
),
...
SELECT
  ...
  CASE
    WHEN latest_portfolio.portfolio_id IS NULL THEN 'warn'
    WHEN latest_portfolio.portfolio_id = 'arena_go_default' AND resolved_startup_identity.bot_name IS NOT NULL THEN 'warn'
    ELSE 'pass'
  END,
  COALESCE(latest_portfolio.portfolio_id, 'missing'),
  'latest live portfolio_snapshot.portfolio_id is the exact resolved ArenaGo bots[].name after broker sync; seed default may exist before startup sync',
  jsonb_build_object('latest_snapshot_created_at', latest_portfolio.created_at, 'resolved_bot_name', resolved_startup_identity.bot_name)
...
VALUES (
  ...
  'info',
  'product_runtime_audit_fixes_applied',
  'Product runtime audit fixes added controlled full-pipeline proof hooks, portfolio identity readiness, future data flags and LLM empty-items handling.',
  'migration',
  '017_product_runtime_audit_fixes',
  ARRAY['controlled_pipeline', 'portfolio_identity', 'future_data_quality', 'llm_empty_items_valid'],
  '{"migration":"017_product_runtime_audit_fixes.sql"}'::jsonb
);
